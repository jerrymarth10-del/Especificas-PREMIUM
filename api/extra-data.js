...
function card(id,img,alt){
  return '<article class="card jr-art-card" onclick="openGate(\''+id+'\')" role="button" tabindex="0" onkeydown="if(event.key===\'Enter\'||event.key===\' \'){event.preventDefault();openGate(\''+id+'\')}" aria-label="'+esc(alt)+'"><img src="'+img+'" alt="'+esc(alt)+'" width="180" height="270" decoding="async" loading="eager"><div class="jr-art-hit" aria-hidden="true"></div></article>';
}
function area(opts){
